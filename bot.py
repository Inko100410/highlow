# main.py — Рекламное Казино v5.1 (АДМИНКА РАБОТАЕТ 100%)
...
def load_data():
    ...
    return {
        ...
        "posts": [],
        "banned_users": [],
        "admins": MASTER_ADMINS.copy(),  # Сразу копируем мастер-админов
        "stats": {
            "total_attempts": 0,
            "total_wins": 0,
            "total_posts_sent": 0
...
def is_admin(user_id):
    ...
    # Сначала проверяем мастер-админов
    if user_id_str in [str(a) for a in MASTER_ADMINS]:
        return True
    # Потом проверяем в списке админов из базы
...
def admin_panel(message):
    ...
    # ОТЛАДКА: пишем в логи
    print_log("INFO", f"Попытка входа в админку: user_id={user_id}")
    print_log("INFO", f"is_admin={is_admin(user_id)}")
    print_log("INFO", f"MASTER_ADMINS={MASTER_ADMINS}")
    print_log("INFO", f"data[admins]={data.get('admins', [])}")
...
def debug(message):
    ...
    text += f"В MASTER_ADMINS: {user_id in MASTER_ADMINS}\n"
    text += f"MASTER_ADMINS: {MASTER_ADMINS}\n"
    text += f"В data['admins']: {str(user_id) in data.get('admins', [])}\n"
    text += f"data['admins']: {data.get('admins', [])}\n"
    text += f"is_admin(): {is_admin(user_id)}\n"
    text += f"is_master_admin(): {is_master_admin(user_id)}"
