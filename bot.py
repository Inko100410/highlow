# main.py — Рекламное Казино v5.0 (ВСЁ ИСПРАВЛЕНО)
...
def get_user_display_name(user_id):
    """Возвращает имя пользователя для отображения (ИСПРАВЛЕНО)"""
    user_id = str(user_id)
    ...
    # Если ничего нет - пытаемся получить через API
    try:
    ...
    # В крайнем случае - последние 4 цифры ID
    ...
    if len(args) > 1:
        ...
        if referrer_id != str(user_id):
            ...
            if user and not user["referrer"]:
                ...
                if referrer:
                    # Проверка на максимум 10 рефералов (ДОБАВЛЕНО)
...
# ========== АДМИН-КОМАНДЫ ДЛЯ УПРАВЛЕНИЯ ШАНСАМИ (ИСПРАВЛЕНО) ==========
...
def set_rating(message):
    ...
    try:
        if len(args) == 3:
            ...
            # Если указано только значение - меняем себе
        ...
        # Уведомляем игрока, если это не сам админ
...
# ========== ПРИЕМ ПОСТОВ (УДАЛЕНО УМЕНЬШЕНИЕ УДАЧИ) ==========
...
def receive_post(message):
    ...
    if message.text:
        ...
        if is_admin(user_id):
            ...
            user["total_posts"] += 1
            # УДАЛЕНО: user["luck"] = max(1.0, user["luck"] - 1.0)  # удача НЕ уменьшается
            save_data(data)
